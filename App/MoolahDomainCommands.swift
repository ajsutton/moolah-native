...
struct MoolahDomainCommands: Commands {
  ...
  /// A Go-menu navigation item. `ForEach` identity is on `title`.
  ...
  private static let sidebarNavItems: [NavItem] = [
    NavItem(title: "Transactions", key: "1", selection: .allTransactions),
    NavItem(title: "Scheduled", key: "2", selection: .upcomingTransactions),
    NavItem(title: "Categories", key: "3", selection: .categories),
    NavItem(title: "Reports", key: "4", selection: .reports),
    NavItem(title: "Analysis", key: "5", selection: .analysis),
  ]
  ...
  var body: some Commands {
    ...
    CommandMenu("Go") {
      ForEach(Self.sidebarNavItems, id: \.title) { item in
        Button(item.title) { sidebarSelection?.wrappedValue = item.selection }
