...
struct TransactionStoreTests {
    ...
    @Test func testFilterByAccountId() async throws {
        ...
        let transactions = [
            Transaction(type: .expense, date: makeDate("2024-01-01"), accountId: accountId, amount: -1000, payee: "Mine"),
            Transaction(type: .expense, date: makeDate("2024-01-02"), accountId: otherAccountId, amount: -2000, payee: "Other"),
            Transaction(type: .transfer, date: makeDate("2024-01-03"), accountId: otherAccountId, toAccountId: accountId, amount: -5000, payee: "Transfer In"),
        ]
        let repository = InMemoryTransactionRepository(initialTransactions: transactions)
        let store = TransactionStore(repository: repository)
    ...
    @Test func testSortedByDateDescending() async throws {
        let transactions = [
            Transaction(type: .expense, date: makeDate("2024-01-01"), accountId: accountId, amount: -1000, payee: "Oldest"),
            Transaction(type: .expense, date: makeDate("2024-01-15"), accountId: accountId, amount: -2000, payee: "Middle"),
            Transaction(type: .expense, date: makeDate("2024-01-30"), accountId: accountId, amount: -3000, payee: "Newest"),
        ]
        let repository = InMemoryTransactionRepository(initialTransactions: transactions)
        let store = TransactionStore(repository: repository)
