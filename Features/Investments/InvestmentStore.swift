...
final class InvestmentStore {
  ...
  func annualizedReturnRate(currentValue: MonetaryAmount) -> Double {
    ...
    let targetCents = Double(currentValue.cents)
    let balanceCents = dailyBalances.map { (date: $0.date, balance: Double($0.balance.cents)) }

    guard let firstBalance = balanceCents.first, firstBalance.balance != 0 else { return 0 }

    let calculateFutureValue = { (rate: Double) -> Double in
      var prevBalance = balanceCents[0].balance
      var val = prevBalance
      var date = balanceCents[0].date

      for i in 1...balanceCents.count {
        let nextDate = i < balanceCents.count ? balanceCents[i].date : latestValue.date
        let nextBalance = i < balanceCents.count ? balanceCents[i].balance : prevBalance
    ...
    while calculateFutureValue(high) < targetCents {
      high *= 2
      if high > 1000 { return .infinity }
    }

    // Ensure low is below the minimum possible return
    while calculateFutureValue(low) > targetCents {
      low *= 2
    ...
    for _ in 0..<100 {
      ...
      if low > high {
        return guess * 100
      } else if abs(fv - targetCents) < 1 {
        return guess * 100
      } else if fv > targetCents {
        high = guess - 0.0001
      } else {
        low = guess + 0.0001
      }
...
func mergeChartData(
  ...
  var dataByDate: [Date: (value: Int?, balance: Int?)] = [:]
  ...
  for value in values {
    ...
    dataByDate[value.date] = (value: value.value.cents, balance: existing?.balance)
  }
  ...
  for balance in balances {
    ...
    dataByDate[balance.date] = (value: existing?.value, balance: balance.balance.cents)
  }
  ...
  if let startDate {
    if let sv = startValue {
      ...
      dataByDate[startDate] = (
        value: existing?.value ?? sv.value.cents,
    ...
    if let sb = startBalance {
      ...
      dataByDate[startDate] = (
        ...
        balance: existing?.balance ?? sb.balance.cents
      )
    }
  }
